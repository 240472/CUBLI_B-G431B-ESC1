...
void CAN_ConfigFilters(FDCAN_HandleTypeDef *hfdcan)
{
    ...
    filter.FilterID2 = 0x0FF | CAN_DEVICE;   // Accept IDs only for the specified device
...
void CAN_Procces_Received_Messages(void)
{
...
void CAN_SendFrame(FDCAN_HandleTypeDef *hfdcan, uint8_t data[8], uint32_t messageIdentifier)
{
    FDCAN_TxHeaderTypeDef TxHeader;

    TxHeader.Identifier = messageIdentifier | CAN_DEVICE;
