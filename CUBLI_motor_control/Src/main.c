...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile uint16_t canWriteIndex = 0;
volatile uint16_t canReadIndex = 0;
...
int main(void)
{
  ...
  MX_NVIC_Init();
  /* USER CODE BEGIN 2 */
  CAN_ConfigFilters(&hfdcan1);
  CAN_Start_and_Notify(&hfdcan1);
  ...
  while (1)
  {
	send_speed = CAN_Procces_Received_Messages();
	if(send_speed == 1)
	{
	...
	data[0] ^= 1;
	HAL_Delay(500);
	CAN_SendFrame(&hfdcan1, data2, FDCAN_DLC_BYTES_8, 0x7E4);
	data2[0] ^= 1;*/
    /* USER CODE END WHILE */
